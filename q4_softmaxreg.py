...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    ind_sen = [tokens[s] for s in sentence]
    sentVector = np.mean(wordVectors[ind_sen,:], axis=0)
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    d_soft[range(N),labels] =  d_soft[range(N),labels] - 1.0
    den= N + regularization*weights
    grad = np.dot(features.transpose(),d_soft) / den
    ### END YOUR CODE
